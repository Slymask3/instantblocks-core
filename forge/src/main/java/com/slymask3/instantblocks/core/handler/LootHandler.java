...
@Mod.EventBusSubscriber(modid = Core.FORGE_MOD_ID, bus = Mod.EventBusSubscriber.Bus.FORGE)
public class LootHandler {
    ...
    private static LootPool injectPool(String name) {
        return LootPool.lootPool().add(LootTableReference.lootTableReference(new ResourceLocation(Core.FABRIC_MOD_ID, "inject/" + name)).setWeight(1)).setBonusRolls(UniformGenerator.between(0, 1)).name(Core.MOD_BASE+"_inject_pool").build();
    }
}