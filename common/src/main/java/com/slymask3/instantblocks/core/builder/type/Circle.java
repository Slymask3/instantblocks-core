        ...
        for(int row = 0; row <= 2 * radius; row++) {
            for(int col = 0; col <= 2 * radius; col++) {
                ...
                if(distance > radius - 0.4 && distance < radius + 0.6) {
                    Single.setup(builder, world, x + row - radius, y, z + col - radius).setBlock(outerBlockType).setCharge(charge).queue(this.priority,this.replace);
                } else if(distance < radius - 0.3) {
                    Single.setup(builder, world, x + row - radius, y, z + col - radius).setBlock(innerBlockType).setCharge(charge).queue(this.priority,this.replace);
                }
            }
        }
    }
